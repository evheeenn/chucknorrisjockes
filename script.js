...
let render = (data) => {
  data.forEach((el) => {
    ...
    jokeIdHref.setAttribute(
      "href",
      `https://api.chucknorris.io/jokes/${el.id}`
    );
    ...
    if (el.categories.length !== 0) {
      ...
      let jokeCategoryText = document.createElement("p");
      jokeCategoryText.classList.add("joke-category-text");
      jokeCategoryText.textContent = el.categories.toString().toUpperCase();

      jokeInfo.appendChild(jokeCategory);
      jokeCategory.appendChild(jokeCategoryText);
    }
  });
};
...
let clickHeart = (data) => {
  let img = document.querySelector(
    `.joke-block[data-id="${data.id}"] .like-img`
  );
  let store = getStore();
